...
<div class="container showmain border rounded" style="padding: 2rem;">
  <div class="row">
    <div class="recipetitle page-header" style="font-size: 50px ; text-align: left">{{ $recipe->title }}</div>
    ...
    <div class="showbody">
      ...
      <table class="table">
        ...
        <tr><td>{{ $materrial->name }}</td><td>{{ empty($materrial->degree) ? '' : $materrial->degree }}</td><td>{{ $materrial->quantity }}
          <td><button class="btn btn-default"><i class="fas fa-shopping-cart"></i> 購入</button></td></td></tr>
        @endforeach
      </table>
    </div>
  </div>
