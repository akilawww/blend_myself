  ...
  @else
    @if ($nice->isEmpty())
      <form method="POST" action="{{ url('/nice/add') }}">
        ...
        <input type="hidden" name="recipe_id" value="{{ $recipe->id }}">
        <input type="hidden" name="user_id" value="{{ Auth::id() }}">
        <button type="submit" class="btn btn-default hoge"><i class="far fa-thumbs-up"></i>いいね</button>
      </form>
    @else
      <form method="POST" action="{{ url('/nice/remove') }}">
        ...
        <input type="hidden" name="recipe_id" value="{{ $recipe->id }}">
        <input type="hidden" name="user_id" value="{{ Auth::id() }}">
        <button type="submit" class="btn btn-default hoge"><i class="fas fa-thumbs-up"></i>いいね削除</button>
      </form>
    @endif
  @endif
  {{ count($niceCount) }}件
